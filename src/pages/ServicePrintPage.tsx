    ...
    try {
      ...
      if (parsed.items && Array.isArray(parsed.items)) {
        const subtotal = parsed.items.reduce(
          (sum: number, item: { qty?: number; quantity?: number; price?: number; unit_price?: number }) => {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      await generatePDF({
        ...
        filename: `Ficha-${service.code}.pdf`,
        format: "a4",
        scale: 2,
        onclone: (clonedDoc) => {
          const el = clonedDoc.querySelector(".print-sheet") as HTMLElement;
          if (el) {
            el.style.margin = "0";
            el.style.position = "static";
          }
          const noPrints = Array.from(clonedDoc.querySelectorAll(".no-print")) as HTMLElement[];
    ...
    <div className="print-page">
      ...
      <div ref={printSheetRef} className="print-sheet relative bg-white">
        ...
        <div
          ...
          aria-hidden="true"
        >
          <img src={tecnofrioLogoIcon} alt="" className="w-[180mm] h-[180mm] object-contain" />
        </div>

        {/* Header */}
        <div className="flex items-start justify-between mb-1.5 relative z-10">
          <div className="flex flex-col">
            <img src={tecnofrioLogoFull} alt="TECNOFRIO" className="h-10 object-contain" />
        ...
        <section className="mb-2">
          ...
          <div className="grid grid-cols-3 gap-x-4 gap-y-1 text-xs">
            ...
            <div>
              ...
              <span className="font-medium">{service.service_location === "cliente" ? "Visita" : "Oficina"}</span>
            </div>
            <div>
          ...
          <>
            ...
            <section className="bg-amber-50 border border-amber-200 rounded p-1.5 text-xs">
              ...
              <p className="text-amber-700 leading-tight text-[10px]">
                Os equipamentos só podem permanecer nas instalações por <strong>30 dias</strong> após conclusão e
                notificação. Após este prazo, a empresa <strong>não se responsabiliza</strong>
